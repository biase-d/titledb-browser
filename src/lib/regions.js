/**
 * Maps country codes to human-readable labels with high granularity
...
export function getRegionLabel(regions) {
    ...
    const europe = new Set(['GB', 'FR', 'DE', 'IT', 'ES', 'NL', 'PT', 'RU', 'AT', 'BE', 'BG', 'CH', 'CY', 'CZ', 'DK', 'EE', 'FI', 'GR', 'HR', 'HU', 'IE', 'IL', 'LT', 'LU', 'LV', 'MT', 'NO', 'PL', 'RO', 'SE', 'SI', 'SK', 'ZA']);
    ...
    const strictEurope = new Set([...europe].filter(c => !['ZA', 'AU', 'NZ'].includes(c)));
    if (isStrict(strictEurope)) return 'Europe';
    ...
    const hasEurope = regions.some(r => strictEurope.has(r));
    ...
    if (isStrict(new Set([...strictEurope, ...oceania]))) return 'Europe & Oceania';

	if (count <= 3) return regions.join(', ');
    if (count <= 5) return 'Multi-Region';
