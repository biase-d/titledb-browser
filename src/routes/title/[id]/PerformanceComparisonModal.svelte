	...
	function toggleSelection(index) {
		if (selectedIndices.has(index)) {
			...
			if (selectedIndices.size < MAX_SELECTIONS) {
				...
				alert(`You can select a maximum of ${MAX_SELECTIONS} versions to compare`);
			}
		}
	}
...
{#if show}
	<div class="modal-overlay" onclick={() => show = false} transition:fade={{ duration: 150 }} role="button" tabindex="0">
						...
						<thead>
							<tr>
								...
								{#each comparisonData as profile}
									<th>{profile.suffix ? `${profile.gameVersion} (${profile.suffix})` : profile.gameVersion}</th>
								{/each}
							</tr>
						</thead>
						<tbody>
							<tr class="mode-row"><td colspan={comparisonData.length + 1}>Docked</td></tr>
							<tr class:has-diff={hasDifference('resolution', 'docked')}>
								<td>Resolution</td>
								{#each comparisonData as profile}
									<td>{formatResolution(profile.profiles?.docked)}</td>
								{/each}
							</tr>
							<tr class:has-diff={hasDifference('framerate', 'docked')}>
								<td>Framerate</td>
								{#each comparisonData as profile}
									<td>{formatFramerate(profile.profiles?.docked)}</td>
								{/each}
							</tr>
							<tr class="mode-row"><td colspan={comparisonData.length + 1}>Handheld</td></tr>
							<tr class:has-diff={hasDifference('resolution', 'handheld')}>
								<td>Resolution</td>
								{#each comparisonData as profile}
									<td>{formatResolution(profile.profiles?.handheld)}</td>
								{/each}
							</tr>
							<tr class:has-diff={hasDifference('framerate', 'handheld')}>
								<td>Framerate</td>
								{#each comparisonData as profile}
									<td>{formatFramerate(profile.profiles?.handheld)}</td>
								{/each}
							</tr>
						</tbody>
					</table>
